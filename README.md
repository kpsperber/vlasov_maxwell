...
## Numerical Methods
This system is solved using a five step algorithm. First, the total electric field is assembled from the plasma field and the external laser field. Second, the Vlasov equation is solved implicity to obtain the updated distribution function. Next, the charge density is computed by integrating the distribution function over velocity space. Then, the Poisson equation is solved for the electrostatic potential. Finally, the electric field is computed from the potential gradient. This algorithm is broken down further in the coming paragraphs. 
...
Once the electric potential is obtained, the electric field generated by the laser is computed from the gradient of the potential,

$$\vec{E}_{charge} = -\nabla \phi.$$

This field is then superimposed with the Electric field produced by the laser. 
